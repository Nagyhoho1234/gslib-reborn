...
c
      write(*,9998) VERSION
 9998 format(/' GAMV - GSLIB2-OPT v',f7.1, ' Finished'/)
      stop
      end
...
c
      write(*,9999) VERSION
 9999 format(/' GAMV - GSLIB2-OPT v',f7.1,' (64-bit, OpenMP)'/
...
c
      ...
      if(nvar.gt.MAXVAR)  stop 'nvar is too big: check gamv.inc'
      backspace lin
      ...
      if(nlag.gt.MAXLAG)  stop 'nlag is too big: check gamv.inc'
      ...
      if(ndir.gt.MAXDIR)  stop 'ndir is too big: check gamv.inc'
     ...
     +                         dip(i),dtol(i),bandwd(i)
      ...
      if(nvarg.gt.MXVARG) stop 'nvarg is too big: check gamv.inc'
...
c$OMP+PRIVATE(i,j,dx,dy,dz,dxs,dys,dzs,hs,h,lagbeg,lagend,ilag)
c$OMP+PRIVATE(id,dxy,dcazm,band,dcdec,omni,iv,it,ii,vrh,vrt)
c$OMP+PRIVATE(vrtpr,vrhpr,il,gamma)
