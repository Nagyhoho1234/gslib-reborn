...
c
      ...
      close(ldbg)
      write(*,9998) VERSION
 9998 format(/' LUSIM - GSLIB2-OPT v',f7.1, ' Finished'/)
      stop
      end
...
c
      write(*,9999) VERSION
 9999 format(/' LUSIM - GSLIB2-OPT v',f7.1,' (64-bit, OpenMP)'/
...
c
 ...
 100  format(/,'LUSIM Debugging file',/)
      ...
      write(lout,101)
 101  format('LUSIM Output',/,'1',/,'simulated values')
c
...
c
 2    read(lin,*,end=3,err=99) (var(j),j=1,nvari)
      ...
      if(nd.gt.MAXDAT) then
            write(*,*) ' ERROR: number of data exceed MAXDAT',MAXDAT
            stop
      end if
      if(ix.ge.1) then
            x(nd)  = var(ix)
...
c-----------------------------------------------------------------------
      include  'lusim.inc'
      real      xg(MAXXY),yg(MAXXY),zg(MAXXY),y2(MAXXY),w2(MAXXY),
     +          c11(MAXDAT,MAXDAT),c12(MAXDAT,MAXXY),c22(MAXXY,MAXXY),
     +          u12(MAXDAT,MAXXY),c22i(MAXXY,MAXXY),l11(MAXDAT,MAXDAT),
     +          l11inv(MAXDAT,MAXDAT),l21v1(MAXXY),l22(MAXXY,MAXXY),
     +          le(MAXDAT),slsk(MAXXY),v1(MAXDAT)
...
c
c Compute the simulation at each grid point (parallelized):
c
...
!$OMP END PARALLEL DO
c
c Write the simulation out to file (buffered output):
c
            ic = 0
            do iz=1,nz
                  do iy=1,ny
