...
c
      parameter(MAXDAT = 5000000, MAXCUT = 500)
c
c Fixed parameters:
c
      parameter(MV=20,UNEST=-99.,EPSLON=1.0e-6,VERSION=2026.1)
      character distin*40,datafl*40,outfl*40,str*40
      real      ccdf(MAXCUT),ccdf1(MAXCUT),ccdf2(MAXCUT),ccut(MAXCUT),
     +          ccut1(MAXCUT),cdf(MAXDAT),cut(MAXDAT),val(MV),
     +          ltpar,mpar,utpar
      integer   ltail,middle,utail
      logical   testfl
      data      lin/1/,lout/2/
c
...
c
      write(*,9999) VERSION
 9999 format(/' POSTIK - GSLIB2-OPT v',f7.1,' (64-bit, OpenMP)'/
...
c
      ...
      write(*,*) ' input distributions = ',distin
...
c
      write(*,9998) VERSION
 9998 format(/' POSTIK - GSLIB2-OPT v',f7.1, ' Finished'/)
      stop
