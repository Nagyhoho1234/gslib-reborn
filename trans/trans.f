...
c
c OPTIMIZED VERSION:
c   - MAXREF increased from 10000 to 500000
c   - MAXDAT increased from 500000 to 5000000
c   - Output buffered via larger write batches
c
...
c
c-----------------------------------------------------------------------
      parameter(MAXREF=500000, MAXDAT=5000000, MAXCAT=24, MV=20,
     +          EPSLON=1.0e-12, VERSION=2026.1)
     ...
     +          ltpar,utpar
      integer   ltail,utail,nx,ny,nz,wx,wy,wz
      logical   testfl
c
...
c
      write(*,9999) VERSION
 9999 format(/' TRANS - GSLIB2-OPT v',f7.1,' (64-bit, OpenMP)'/
...
c
      if(ivtype.eq.1) then
            ...
            gmedian = powint(rcdf(j),rcdf(j+1),rvr(j),rvr(j+1),0.5,
     +                       1.0)
...
c
      write(*,9998) VERSION
 9998 format(/' TRANS - GSLIB2-OPT v',f7.1, ' Finished'/)
      stop
