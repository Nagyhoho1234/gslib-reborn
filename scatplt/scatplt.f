...
c
c-----------------------------------------------------------------------
      parameter (MAXDAT = 500000, MV=100, EPSLON=1.0e-21, VERSION=2.000)

      character  datafl*40,outfl*40,title*40,str*80,xlab*24,ylab*24
      real       vr1(MAXDAT),vr2(MAXDAT),vr3(MAXDAT),wt(MAXDAT),var(MV)
      logical    testfl
c
...
c
      ...
      write(str(1:12),'(f12.2)') vr1m
      call pstext(x2,yy,12,str,ts,it,rt,0)
      yy = yy - y1

      str = 'std. dev.'
      call pstext(x1,yy,9,str,ts,it,rt,2)
      write(str(1:12),'(f12.2)') sqrt(max(vr1v,0.0))
      call pstext(x2,yy,12,str,ts,it,rt,0)
      yy = yy - y2

      str = 'Y Variable: mean'
      call pstext(x1,yy,16,str,ts,it,rt,2)
      write(str(1:12),'(f12.2)') vr2m
      call pstext(x2,yy,12,str,ts,it,rt,0)
      yy = yy - y1

      str = 'std. dev.'
      call pstext(x1,yy,9,str,ts,it,rt,2)
      write(str(1:12),'(f12.2)') sqrt(max(vr2v,0.0))
      call pstext(x2,yy,12,str,ts,it,rt,0)
      yy = yy - y2

      str = 'correlation'
      call pstext(x1,yy,11,str,ts,it,rt,2)
      write(str(1:12),'(f12.2)') corr
      call pstext(x2,yy,12,str,ts,it,rt,0)
      yy = yy - y1

      str = 'rank correlation'
      call pstext(x1,yy,16,str,ts,it,rt,2)
      write(str(1:12),'(f12.2)') rr
      call pstext(x2,yy,12,str,ts,it,rt,0)
      yy = yy - y2
