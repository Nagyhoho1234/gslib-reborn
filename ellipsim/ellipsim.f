...
c
c-----------------------------------------------------------------------
      parameter(MAXXYZ =10000000, MAXELP =  25)
      parameter(KORDEI=12,MAXOP1=KORDEI+1,MAXINT=2**30,MAXROT=MAXELP,
     +          EPSLON=1.0e-10,VERSION=2026.1)
c
...
c
      write(*,9999) VERSION
 9999 format(/' ELLIPSIM - GSLIB2-OPT v',f7.1,' (64-bit, OpenMP)'/
...
c
      ...
      read (*,'(a40)') str
      if(str(1:1).eq.' ')str='ellipsim.par                            '
...
c
 ...
 21         continue
c
c Write out simulation and go back for another (buffered output):
c
            do i=1,nxyz
                  write(lout,'(i2)') sim(i)
            end do
...
c
      close(lout)
      write(*,9998) VERSION
 9998 format(/' ELLIPSIM - GSLIB2-OPT v',f7.1, ' Finished'/)
      stop
