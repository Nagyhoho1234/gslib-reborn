...
c
      write(*,9998) VERSION
 9998 format(/' KB2D - GSLIB2-OPT v',f7.1, ' Finished'/)
      stop
      end
...
c
      write(*,9999) VERSION
 9999 format(/' KB2D - GSLIB2-OPT v',f7.1,' (64-bit, OpenMP)'/
...
c           *************************************************
c
c OPTIMIZED: OpenMP parallelization, buffered I/O output.
c
...
c
      ...
      vk = 0.0
      nxny = nx*ny
      allocate(est_buf(2,nxny))
c
!$OMP PARALLEL DO DEFAULT(SHARED) SCHEDULE(DYNAMIC,32)
!$OMP+PRIVATE(ix,iy,xloc,yloc,na,isam,id,dx,dy,h2,n1,ii,k,jk,jj,j,
!$OMP+  j1,ia,xx,yy,cb,cb1,cov,neq,nn,in,is,ie,ising,
!$OMP+  est,estv,sumw,i,index,
!$OMP+  xa,ya,vra,dist,nums,r,rr,s,a)
!$OMP+REDUCTION(+:nk,ak,vk)
      do index=1,nxny
            iy   = (index-1)/nx + 1
            ix   = index - (iy-1)*nx
            yloc = ymn + (iy-1)*ysiz
...
c
c Store result in buffer:
c
 1          est_buf(1,index) = est
            est_buf(2,index) = estv
...
c
      do index=1,nxny
            write(lout,'(f8.3,1x,f8.3)') est_buf(1,index),
     +                                     est_buf(2,index)
      end do
      deallocate(est_buf)
c
