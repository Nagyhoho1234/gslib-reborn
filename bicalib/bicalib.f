...
c
c OPTIMIZED VERSION:
c   - Eliminated backspace/re-read I/O pattern in output stage
c   - All data lines read into memory, processed and written in batch
c   - MAXDAT increased from 10000 to 5000000
c   - MAXLEN increased to 512
c
...
c
c      MAXDAT       maximum number of calibration data
c      MXUCUT       maximum number of cutoffs on u
c      MXVCUT       maximum number of cutoffs on v
c
c
c
c Original: H. Zhu                                   Date:     July 1990
c-----------------------------------------------------------------------
c
c User Adjustable Parameters:
c
      parameter(MAXDAT=500000, MXUCUT=21, MXVCUT=21)
      parameter(MAXREC=500000)
c
c Fixed Parameters:
c
      parameter(MAXLEN=512, VERSION=2026.1)
c
c Variable declaration:
c
      real      u(MAXDAT),v(MAXDAT),wt(MAXDAT),ucut(0:MXUCUT+1),
     +          vcut(0:MXVCUT+1),fract(MXUCUT),lcdf(MXUCUT),
     +          pdf(MXVCUT+1,MXUCUT+1),yx(MXVCUT+1,MXUCUT+1),
     +          soft(MXUCUT,0:1,MAXDAT),em(MXUCUT,0:1),vm(MXUCUT,0:1),
     +          softw(MXUCUT,0:1,MAXDAT),b(MXUCUT),var(20)
      integer   nd,ncutu,ncutv,nm(MXUCUT,0:1)
      character datafl*512,calfl*512,outfl*512,repfl*512,
     +          mbsimfl*512,str*512
      character lines(MAXREC)*512
      integer   lostr(MAXREC),nrec
      real      outlcdf(MXUCUT,MAXREC)
      logical   testfl
c
...
c
      write(*,9999) VERSION
 9999 format(/' BICALIB - GSLIB2-OPT v',f7.1,' (64-bit, OpenMP)'/
...
c
      ...
      read(lin,'(a40)',err=98) calfl
      call chknam(calfl,40)
      write(*,*) ' data file = ',calfl
      ...
      read(lin,'(a40)',err=98) mbsimfl
      call chknam(mbsimfl,40)
      write(*,*) ' output file for SISIM = ',mbsimfl

      read(lin,'(a40)',err=98) repfl
      call chknam(repfl,40)
      write(*,*) ' report file = ',repfl
...
c
      if(ncutu.gt.MXUCUT)  stop 'too many cutoffs on primary variable'
      if(ncutv.gt.MXVCUT)  stop 'too many cutoffs on secondary variable'
c
...
c
c The calibration file exists so open the file and read in the header
c information.
c
      write(*,*)
...
c
 3    read(lin,*,end=4,err=99) (var(j),j=1,nvari)
      ...
      nd    = nd + 1
      if(nd.ge.MAXDAT) then
            write(*,*) ' ERROR: not enough storage - increase MAXDAT'
            stop
      endif
      u(nd)  = var(ivu)
      v(nd)  = var(ivv)
...
c OPTIMIZED: Read all lines into memory, process in memory, write batch
c
...
c
c OPTIMIZED: Read ALL data lines into memory in single pass
c
      nrec = 0
 40   read(lin,'(a)',end=41) str
      nrec = nrec + 1
      if(nrec.gt.MAXREC) then
            write(*,*) 'ERROR: too many records, max = ',MAXREC
            stop
      endif
      lines(nrec) = str
      call strlen(str,MAXLEN,lostr(nrec))
      go to 40
 41   continue
      close(lin)
      write(*,*) ' Read ',nrec,' data records into memory'
c
c OPTIMIZED: Process all records in memory (no backspace I/O)
c
      do irec=1,nrec
            read(lines(irec),*,err=97) (var(j),j=1,nvari)
            vval = var(isec)
 ...
 42               continue
                  do i=1,ncutu
                        outlcdf(i,irec) = yx(icls,i)
                  end do
            end if
      end do
c
c OPTIMIZED: Write all output in single buffered pass
c
      do irec=1,nrec
            write(lout,'(a,1x,24(f8.4))') lines(irec)(1:lostr(irec)),
     +            (outlcdf(i,irec),i=1,ncutu)
      end do
c
...
c
      write(*,9998) VERSION
 9998 format(/' BICALIB - GSLIB2-OPT v',f7.1, ' Finished'/)
      stop
