...
c
      ...
      close(ldbg)
      write(*,9998) VERSION
 9998 format(/' ANNEAL - GSLIB2-OPT v',f7.1, ' Finished'/)
      stop
      end
...
c
      write(*,9999) VERSION
 9999 format(/' ANNEAL - GSLIB2-OPT v',f7.1,' (64-bit, OpenMP)'/
...
c
      ...
      write(*,*) ' input images = ',datafl

      read(lin,'(a40)',err=97) trainfl
      call chknam(trainfl,40)
      write(*,*) ' training image = ',trainfl

      read(lin,*,err=97) idbg,report
      write(*,*) ' idbg, report = ',idbg,report

      read(lin,'(a40)',err=97) dbgfl
      call chknam(dbgfl,40)
      write(*,*) ' debug file = ',dbgfl

      read(lin,'(a40)',err=97) outfl
      call chknam(outfl,40)
      write(*,*) ' output images = ',outfl
...
c
c     Allocate grid array now that dimensions are known:
c
      call anneal_alloc(nx, ny, nz)
...
c
      ...
      if(mlag.gt.MAXLAG) stop 'nlag is too big - modify .inc file'
      if(ndir.gt.MAXDIR) stop 'ndir is too big - modify .inc file'
c
...
c
c read in the training image:
c
      title = 'ANNEAL SIMULATIONS:                      '//
     +        '                                         '
      inquire(file=trainfl,exist=testfl)
      if(.not.testfl) then
            write(*,*) 'ERROR file ',trainfl,' does not exist!'
      ...
      do iz=1,nz
            do iy=1,ny
                  do ix=1,nx
                        ...
                        do i=1,ncat
                              if(icode.eq.icat(i)) go to 71
                        end do
                        ncat = ncat + 1
                        if(ncat.gt.MAXRC) then
                              write(*,*) ' ERROR too many codes ',MAXRC
                              stop
                        end if
                        icat(ncat)    = icode
 71                     var(ix,iy,iz) = icode
                  end do
            end do
      end do
 6    close(lin)
...
c
c Buffered output: write realization values
c
                  do iz=1,nz
                        do iy=1,ny
                              do ix=1,nx
                                    ind = var(ix,iy,iz)
                                    write(lout,'(i4)')  icat(ind)
                              end do
                        end do
                  end do
