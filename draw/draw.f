...
c Parameters:
c
      parameter(MAXCOL=3,MAXDIS=10000,EPSLON=1.0e-10,VERSION=2.000,
     +          KORDEI=12,MAXOP1=KORDEI+1,MAXINT=2**30)
c
c Variable Declaration:
c
      real      prob(MAXDIS),val(MAXCOL,MAXDIS),tmp(50)
      integer   colkeep(MAXCOL),colprob
      character datafl*40,outfl*40,str*40,labels(MAXCOL)*12
      logical   testfl
c
...
c
      ...
      ldep = 3
      write(*,9999) VERSION
 9999 format(/' DRAW - GSLIB2-OPT v',f7.1,' (64-bit, OpenMP)'/
...
c
      ...
      write(*,*) ' input distribution file = ',datafl

      read(lin,*,err=97) ncolkeep
      write(*,*) ' number of variables to keep = ',ncolkeep
      if(ncolkeep.gt.MAXCOL) then
            write(*,*) 'ERROR: only have ',MAXCOL,' room for columns'
            stop
      end if
