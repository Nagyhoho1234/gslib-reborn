...
c
c-----------------------------------------------------------------------
      parameter(EPSLON=1.e-12,UNEST=-999.,VERSION=2026.1)
      character datafl*40,outfl*40,str*512
      real, allocatable :: var(:,:,:), CUT(:), CDF(:), buf(:)
      real      meana,meanb
      integer   nxyz, ntot, ioff
      logical   testfl
      data      lin/1/,lout/2/
c
...
c
      write(*,9999) VERSION
 9999 format(/' POSTSIM - GSLIB2-OPT v',f7.1,' (64-bit, OpenMP)'/
...
c
      ...
      write(*,*) ' data file with simulations = ',datafl
      ...
      close(lin)
c
c Allocate arrays based on actual dimensions:
c
      allocate(var(nx,ny,nsim))
      allocate(CUT(nsim))
      allocate(CDF(nsim))
c
c Set up cdf once:
...
c
c Write grid-aware header (original format: ncols + grid dims on line 2)
c Always adds variance as extra column
c
      if(iout.eq.1) then
            write(lout,'(a)') 'E-type and Variance'
            write(lout,106) 2,nx,ny,nz
            write(lout,'(a)') 'E-type'
            write(lout,'(a)') 'variance'
      else if(iout.eq.2) then
            write(lout,102) outpar
            write(lout,106) 4,nx,ny,nz
            write(lout,'(a)') 'prob > cutoff'
            write(lout,'(a)') 'mean > cutoff'
            write(lout,'(a)') 'mean < cutoff'
            write(lout,'(a)') 'variance'
            ...
            write(lout,104) outpar
            write(lout,106) 3,nx,ny,nz
            write(lout,'(a)') 'lower'
            write(lout,'(a)') 'upper'
            write(lout,'(a)') 'variance'
      else if(iout.eq.5) then
            write(lout,'(a)') 'Conditional Variance'
            write(lout,106) 2,nx,ny,nz
            write(lout,'(a)') 'E-type'
            write(lout,'(a)') 'variance'
      end if
 102  format('Probability and mean value > ',f12.4)
 103  format('Z value corresponding to CDF = ',f7.4)
 104  format('Probability Interval = ',f7.4)
 106  format(4x,i2,3x,i4,3x,i4,4x,i4)
c
...
c
c Read ALL data into a flat buffer in single pass, then reorganize:
c
      nxyz = nx * ny * nz
      ntot = nxyz * nsim
      allocate(buf(ntot))
...
c
      do 2 iznow=1,nz
c
c Extract data for this Z level from buffer:
c (Buffer order: sim1[x varies fastest, then y, then z], sim2[...], ...)
c
      do is=1,nsim
            ioff = (is-1)*nxyz + (iznow-1)*nx*ny
            do iy=1,ny
                  do ix=1,nx
                        var(ix,iy,is) = buf(ioff + (iy-1)*nx + ix)
                  end do
            end do
      end do
c
...
c
c Always compute E-type and variance:
c
...
c
c Write E-type + variance?
c
            if(iout.eq.1) then
                  write(lout,'(g14.7,5x,g14.7)') etype,cvar
c
...
c
                  ...
                  write(lout,'(f9.4,1x,f9.4,2x,f9.4,2x,g14.7)')
     +                  prob,meana,meanb,cvar
c
...
c
                  ...
                  write(lout,'(f9.4,2x,g14.7)') zval,cvar
c
...
c
                  ...
                  write(lout,'(f12.4,1x,f12.4,2x,g14.7)')
     +                  zlow,zupp,cvar
c
...
c
 2      continue
c
c Cleanup and finished:
c
      deallocate(var,CUT,CDF,buf)
      write(*,9998) VERSION
 9998 format(/' POSTSIM - GSLIB2-OPT v',f7.1, ' Finished'/)
      stop
