...
c file is described in the documentation (see the example sgsim_fc.par)
c
...
c
      write(*,9998) VERSION
 9998 format(/' SGSIM_FC - GSLIB2-OPT v',f7.1, ' Finished'/)
      stop
      end
...
c
      write(*,9999) VERSION
 9999 format(/' SGSIM_FC - GSLIB2-OPT v',f7.1,' (64-bit, OpenMP)'/
...
cl Varred always 1, it is not needed

             varred=1

            read(lin,*,err=98) maxNsec
            if(maxNsec.gt.MAXSEC) then
             write (*,*) 'Too many secondary data points required'
             write (*,*) 'modify MAXSEC'
             stop
            end if
 ...
 9990       continue
            ...
            write(*,*) ' max. number sec. data to use if LMC = ',maxNsec
...
cl read more variograms
     ...
     +                   ang1(i),ang2(i),ang3(i)
      ...
      if(ktype.ge.6) then
      read(lin,*,err=98) nst(3),c0(3)
      write(*,*) ' nst, c0 = ',nst(3),c0(3)
      do i=MAXNST*2+1,MAXNST*2+nst(3)
        read(lin,*,err=98) it(i),cc(i),ang1(i),ang2(i),ang3(i)
        read(lin,*,err=98) aa(i),aa1,aa2
        anis1(i) = aa1 / max(aa(i),EPSLON)
        anis2(i) = aa2 / max(aa(i),EPSLON)
...
cl Check LMC here !!!!!
      ...
      if(ktype.ge.6) then
        linmod= .true.
        if(nst(2).gt.nst(1).or.nst(2).gt.nst(3)) linmod = .false.
        do i=1,nst(2)
                     index3 = 2*MAXNST+i
                     index2 = MAXNST+i
                     index1 = i
                     if(it(index1).ne.it(index2).or.
     +                it(index3).ne.it(index2).or.
     ...
     +                abs(anis2(index1) - anis2(index2)).gt.EPSLON.or.
     +                abs(aa(index3)    - aa(index2)).gt.EPSLON.or.
     +                abs(ang1(index3)  - ang1(index2)).gt.EPSLON.or.
     +                abs(ang2(index3)  - ang2(index2)).gt.EPSLON.or.
     +                abs(ang3(index3)  - ang3(index2)).gt.EPSLON.or.
     +                abs(anis1(index3) - anis1(index2)).gt.EPSLON.or.
     +                abs(anis2(index3) - anis2(index2)).gt.EPSLON)
     +                linmod = .false.
        end do
...
c
            posdef = .true.
                  if(c0(1).lt.0.0.or.c0(3).lt.0.0.or.
     +              (c0(1)*c0(3)).lt.(c0(2)*c0(2)) ) then
                        posdef = .false.
                  endif
                  do i=1,nst(2)
                      indexii=2*MAXNST+i
                      indexjj=i
                      indexij=MAXNST+i
                        if(cc(indexii).lt.0.0.or.cc(indexjj).lt.0.0.or.
     +                    (cc(indexii)*cc(indexjj)).lt.
     +                    (cc(indexij)*cc(indexij)) ) then
                              posdef = .false.
                        endif
                  end do
        else
...
c
      ...
      write(lout,110)
 110  format('SGSIM Realizations',/,'1',/,'value')
...
c
c OpenMP parallelization: Each realization is independent and runs on
c its own thread with private simulation arrays and RNG state.
c The THREADPRIVATE common blocks (iaco, simwrk, search, clooku,
c krigev) give each thread its own copy of mutable state.
c
c
c
c PROGRAM NOTES:
...
c-----------------------------------------------------------------------
      ...
      logical   testind
c
c OMP: Per-thread seed arrays for ACORN RNG (one per realization)
c
      integer   ixvseed(MAXOP1,1024)
c
c OMP: Shared result array for all realizations (write after parallel)
c
      real, allocatable :: allout(:,:)
      real, allocatable :: allss(:)
      integer, allocatable :: allne(:)
      real, allocatable :: allav(:), allavs(:), allsss(:)
c
c OMP: Accumulator for average variance across realizations
c
      real      sssVAR
c
...
cl Average variance!!!!
      ...
      sssVAR=0
c
c OMP: Allocate shared result storage
c
      allocate(allout(nxyz,nsim))
      allocate(allss(nsim))
      allocate(allne(nsim))
      allocate(allav(nsim))
      allocate(allavs(nsim))
      allocate(allsss(nsim))
c
c OMP: Pre-generate per-realization RNG seeds sequentially
c
      do isim=1,nsim
            do i=1,MAXOP1
                  ixvseed(i,isim) = ixv(i)
            end do
c         Advance RNG by nxyz+1000 draws to create next seed
            do i=1,nxyz+1000
                  p = acorni(idum)
            end do
      end do
...
c OMP: Parallel realization loop with THREADPRIVATE common blocks
c
!$OMP PARALLEL DO
!$OMP+  PRIVATE(isim,ind,ix,iy,iz,in,index,id,id2,xx,yy,zz,
!$OMP+          test,test2,TINY,irepo,gmean,cmean,cstdev,lktype,
!$OMP+          p,xp,ierr,simval,ne,av,avs,ss,sss,ssc,
!$OMP+          imult,nnx,nny,nnz,jx,jy,jz,testind,infoct,
!$OMP+          var,cp,oldcp,w)
!$OMP+  COPYIN(/iaco/,/simwrk/,/search/,/clooku/,/krigev/,
!$OMP+         /iobuf/)
!$OMP+  REDUCTION(+:sssVAR)
!$OMP+  SCHEDULE(DYNAMIC,1)
      do isim=1,nsim
c
c OMP: Set this thread's RNG seed for this realization
c
            do ind=1,MAXOP1
                  ixv(ind) = ixvseed(ind,isim)
            end do
c
c Read in the secondary data distribution for this realization:
c (Note: secondary data re-reading for ktype>=4 with isim>1 is
c  not thread-safe. For that case, run with OMP_NUM_THREADS=1)
c
...
c
     ...
     +               sim(index).lt.(UNEST*2.0)) go to 8
...
c
c Back transform each value into output buffer (buffered I/O):
c
            ne  = nxyz
            av  = 0.0
            avs = 0.0
     ...
     +                                  zmax,ltail,ltpar,utail,utpar)
            ...
            avs  = avs / max(real(ne),1.0)
            do ind=1,nxyz
                  ss  = ss  + (sim(ind)-av )*(sim(ind)-av )
                  sss = sss + (lvm(ind)-avs)*(lvm(ind)-avs)
                  ssc = ssc + (sim(ind)-av )*(lvm(ind)-avs)
            end do

            ss   = ss  / max(real(ne),1.0)
            sss  = sss / max(real(ne),1.0)
            ssc  = ssc / max(real(ne),1.0)
c
c OMP: Store results in shared array (write sequentially after loop)
c
            do ind=1,nxyz
                  allout(ind,isim) = outbuf(ind)
            end do
            allss(isim)  = ss
            allne(isim)  = ne
            allav(isim)  = av
...
!$OMP END PARALLEL DO
c
c Write all realizations sequentially in correct order:
c
      do isim=1,nsim
            do ind=1,nxyz
                  write(lout,'(f12.4)') allout(ind,isim)
            end do
            write(*,   111) isim,allne(isim),allav(isim),allss(isim)
 111        format(/,' Realization ',i3,': number   = ',i8,/,
     +               '                  mean     = ',f12.4,
     +               ' (close to 0.0?)',/,
     +               '                  variance = ',f12.4,
     +               ' (close to gammabar(V,V)? approx. 1.0)',/)
            write(*,*) ' secondary: ',allavs(isim),allsss(isim)
      end do
c
      deallocate(allout,allss,allne,allav,allavs,allsss)
c
      sssVAR=sssVAR/nsim
      write(*,*) 'average variance', sssVAR
c
...
c
c OMP: Covariance table construction - the nested loops can be
c      partially vectorized. Each cova3 call is independent.
c
      nlooku = 0
      do i=-nctx,nctx
      xx = i * xsiz
     ...
     +                 1,MAXROT,rotmat,cmax,covtab(ic,jc,kc))
      if(ktype.ge.5) then
         call cova3(0.0,0.0,0.0,xx,yy,zz,2,nst,MAXNST,c0,it,
     +               cc,aa,1,MAXROT,rotmat,cmax,covtr(ic,jc,kc))
...
cl Adding line here
      ...
      if(ktype.ge.6) then
            call cova3(0.0,0.0,0.0,xx,yy,zz,3,nst,
     +              MAXNST,c0,it,cc,aa,1,MAXROT,
     +              rotmat,cmax,covtSec(ic,jc,kc))
...
c-----------------------------------------------------------------------
      ...
      real  xSec(MAXSEC),ySec(MAXSEC),zSec(MAXSEC)
      integer   iSec(MAXSEC),jSec(MAXSEC),kSec(MAXSEC)
...
cl if MMII calculate add term with covr only!!

               if(ktype.eq.5) then
                  if(j.le.nclose.or.i.le.nclose) then
                        call cova3(x1,y1,z1,x2,y2,z2,2,nst,MAXNST,c0,it,
     +                             cc,aa,1,MAXROT,rotmat,cmax,covr)
...
c
     ...
     +                             cc,aa,1,MAXROT,rotmat,cmax,covr)
                ...
                a(in)=dble(colocorr*colocorr)*cov+
     +                (1-dble(colocorr*colocorr))*covr
...
cl if MMII calculate R covariance, use cov calculated before

            if(ktype.eq.5) then
              if(j.le.nclose) then
                 call cova3(xx,yy,zz,x1,y1,z1,2,nst,MAXNST,c0,it,cc,aa,
     +                       1,MAXROT,rotmat,cmax,covr)
              else
c
c Try to use the covariance look-up (if the distance is in range):
c
     ...
     +                       1,MAXROT,rotmat,cmax,covr)
              ...
              rb(j) = cov
              r(j)  =    dble(colocorr*colocorr)  * cov  +
     +                  (1-dble(colocorr*colocorr)) * covr
            endif
            rr(j) = r(j)
      end do
...
c
c Addition of Collocated Cosimulation Constraint - MMI and MMII:
c
      if(lktype.eq.4.or.lktype.eq.5) then
            sfmin =  1.0e21
            sfmax = -1.0e21
            do i=1,na
                  in    = in + 1
...
c
c Addition of more equations if LMC:
c

      if(lktype.eq.6) then

cl First form array of nearby secondary data using spiral search
cl [x,y,z]Sec - array coordinates, [i,j,k]Sec  - array indices

      numSec=0
      do  il=1,nlooku
            if(numSec.eq.maxNsec) goto 662
            i = ix + (int(ixnode(il))-nctx-1)
            j = iy + (int(iynode(il))-ncty-1)
            k = iz + (int(iznode(il))-nctz-1)
            if(i.lt. 1.or.j.lt. 1.or.k.lt. 1) go to 552
            if(i.gt.nx.or.j.gt.ny.or.k.gt.nz) go to 552
            ind = i + (j-1)*nx + (k-1)*nxy

                  numSec = numSec + 1
                  xSec(numSec) = xmn + real(i-1)*xsiz
                  ySec(numSec) = ymn + real(j-1)*ysiz
                  zSec(numSec) = zmn + real(k-1)*zsiz
                  iSec(numSec) = i
                  jSec(numSec) = j
                  kSec(numSec) = k
...
cl Number of equation if LMC

662      neq=na+numSec

cl Form second part of the matrix if LMC

      do i=1,numSec

cl Terms with crosscovariance
...
c
            ...
            if(j.le.nclose) then
                  ...
                  y1     = y(index)
                  z1     = z(index)
                call cova3(x1,y1,z1,xSec(i),ySec(i),zSec(i),2,
     +                nst,MAXNST,c0,it,cc,aa,1,MAXROT,rotmat,cmax,cov)

            else
c
c It is a previously simulated node :
c
                  index  = j-nclose
                  x1     = cnodex(index)
                  y1     = cnodey(index)
                  ...
                  ii = nctx + 1 + (ix1 - iSec(i))
                  jj = ncty + 1 + (iy1 - jSec(i))
                  kk = nctz + 1 + (iz1 - kSec(i))
                  if(ii.lt.1.or.ii.gt.MAXCTX.or.
     +               jj.lt.1.or.jj.gt.MAXCTY.or.
     +               kk.lt.1.or.kk.gt.MAXCTZ) then
                   call cova3(x1,y1,z1,xSec(i),ySec(i),zSec(i),2,
...
cl Terms with autocovariance of secondary

       do j=1,i
              in = in + 1
              ii = nctx + 1 + (iSec(j) - iSec(i))
              jj = ncty + 1 + (jSec(j) - jSec(i))
              kk = nctz + 1 + (kSec(i) - kSec(i))
                  if(ii.lt.1.or.ii.gt.MAXCTX.or.
     +               jj.lt.1.or.jj.gt.MAXCTY.or.
     +               kk.lt.1.or.kk.gt.MAXCTZ) then
                     call cova3(xSec(i),ySec(i),zSec(i),xSec(j),ySec(j),
     +                       zSec(j),3,nst,MAXNST,c0,it,
     +                       cc,aa,1,MAXROT,rotmat,cmax,cov)
                  else
                       cov = covtSec(ii,jj,kk)
                  endif
                  a(in) = dble(cov)
       end do
...
cl RHS if LMC
     ...
     +     jj.lt.1.or.jj.gt.MAXCTY.or.
     +     kk.lt.1.or.kk.gt.MAXCTZ) then
           call cova3(xx,yy,zz,xSec(i),ySec(i),zSec(i),2,
     +              nst,MAXNST,c0,it,cc,aa,1,MAXROT,rotmat,cmax,cov)
         else
           cov = covtr(ii,jj,kk)
         endif
...
c
      ...
      if(lktype.eq.4.or.ktype.eq.5) then
            ind    = ix + (iy-1)*nx + (iz-1)*nxy
            cmean  = cmean  + real(s(na+1))*lvm(ind)
            cstdev = cstdev - real(s(na+1) *rr(na+1))
      end if

cl More terms with secondary data

      if(lktype.eq.6) then
          do i=1,numSec
            ind    = iSec(i) + (jSec(i)-1)*nx + (kSec(i)-1)*nxy
            cmean  = cmean  + real(s(na+i))*lvm(ind)
            cstdev = cstdev - real(s(na+i) *rr(na+i))
...
c
 ...
 140        format(' Data ',i4,' value ',f8.4,' weight ',f8.4)
            if(lktype.ge.4) write(ldbg,141) lvm(ind),s(na+1)
...
c-----------------------------------------------------------------------
      lun = 99
      open(lun,file='sgsim_fc.par',status='UNKNOWN')
      write(lun,10)
 ...
 36   format(' 0.0   0.0   0.0              ',
     +       '-angles for search ellipsoid')
      write(lun,37)
 37   format('0                             ',
     +       '-ktype: 0=SK,1=OK,2=LVM,3=EXDR,4=MMI,5=MMII,6=LMC')
      write(lun,371)
 371   format('0.7                          ',
     +       '-correl. coeff. if MMI or MMII')

      write(lun,372)
 372   format('9                            ',
     +       '- number of secondary data to use if LMC')
 ...
 39   format('4                             ',
     +       '-  column for secondary variable')
      write(lun,40)
 40   format('1    0.1                      ',
     +       '-nst, nugget effect: primary for LMC or gamma_B for MMII')
      write(lun,41)
 ...
 42   format('         10.0  10.0  10.0     ',
     +       '-a_hmax, a_hmin, a_vert')
      write(lun,401)
 401   format('1    0.1                      ',
     +       '-nst, nugget effect: cross for LMC or gamma_R for MMII')
      write(lun,411)
 411   format('1    0.9  0.0   0.0   0.0     ',
     +       '-it,cc,ang1,ang2,ang3')
      write(lun,412)
 412   format('         10.0  10.0  10.0     ',
     +       '-a_hmax, a_hmin, a_vert')
      write(lun,420)
 420   format('1    0.1                      ',
     +       '-nst, nugget effect: secondary for LMC')
      write(lun,421)
 421   format('1    0.9  0.0   0.0   0.0     ',
     +       '-it,cc,ang1,ang2,ang3')
      write(lun,422)
 422   format('         10.0  10.0  10.0     ',
     +       '-a_hmax, a_hmin, a_vert')
