...
c
      write(*,9999) VERSION
 9999 format(/' ADDCOORD - GSLIB2-OPT v',f7.1,' (64-bit, OpenMP)'/
...
c
      ...
      write(lout,200, advance='no')
 200  format('   X  ','Y  ','Z  ')
      do i=1,nvari
            read(lin,'(a20)',err=99) label
            write(lout,'(a20)',advance='no') label
      end do
	  write(lout,201,advance='yes')
 201  format(' ')
...
c
      ...
      close(lout)
      write(*,9998) VERSION
 9998 format(/' ADDCOORD - GSLIB2-OPT v',f7.1, ' Finished'/)
      stop
