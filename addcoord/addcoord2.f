...
c
c-----------------------------------------------------------------------
      parameter (MAXLEN=132,MAXFIL=64,VERSION=2026.1)
      character str*80,datafl*64,outfl*64,fmt*36,label*20
      logical   testfl
      data      lin/1/,lout/2/
c
...
c
      write(*,9999) VERSION
 9999 format(/' ADDCOORD2 - GSLIB2-OPT v',f7.1,' (64-bit, OpenMP)'/
...
c
c Write proper GSLIB header: title, ncols, variable names
c
      write(lout,'(a)') 'ADDCOORD Output'
      write(lout,'(i4)') nvari+3
      write(lout,'(a)') 'X'
      write(lout,'(a)') 'Y'
      write(lout,'(a)') 'Z'
      do i=1,nvari
...
c
      ...
      close(lout)
      write(*,9998) VERSION
 9998 format(/' ADDCOORD2 - GSLIB2-OPT v',f7.1, ' Finished'/)
      stop
