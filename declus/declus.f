...
c
c-----------------------------------------------------------------------
        parameter(MAXDAT=5000000,MAXCEL=5000000,MAXLEN=132,
     +            VERSION=2026.1)

        real      x(MAXDAT),y(MAXDAT),z(MAXDAT),wt(MAXDAT),vr(MAXDAT),
     +            wtopt(MAXDAT),cellwt(MAXCEL),var(20)
        integer   index(MAXDAT)
        character datafl*40,sumfl*40,outfl*40,str*40,strlin*132
        logical   min,testfl
...
c
      write(*,9999) VERSION
 9999 format(/' DECLUS - GSLIB2-OPT v',f7.1,' (64-bit, OpenMP)'/
...
c
c check the array MAXCEL dimensions:
c
            if(ncellt.gt.real(MAXCEL)) then
                  write(*,201) ncellt,MAXCEL
 201              format('ERROR: array dimensions too small MAXCEL'/
     +             ' number of cells = ',i12, ' array size = ',i12,/
     +             ' check for outliers - increase cmin and/or MAXCEL')
                  stop    
            end if
c
...
c
      ...
      close(lsum)
      write(*,9998) VERSION
 9998 format(/' DECLUS - GSLIB2-OPT v',f7.1, ' Finished'/)
      stop
