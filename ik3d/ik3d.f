...
c
      ...
      close(lout)
      write(*,9998) VERSION
 9998 format(/' IK3D - GSLIB2-OPT v',f7.1, ' Finished'/)
      stop
      end
...
c-----------------------------------------------------------------------
      ...
      integer   ivrs(MAXCUT)
      real      var(MV)
      character datafl*40,softfl*40,outfl*40,dbgfl*40,jackfl*40,str*40
      logical   testfl
c
...
c
      write(*,9999) VERSION
 9999 format(/' IK3D - GSLIB2-OPT v',f7.1,' (64-bit, OpenMP)'/
...
c
            nd = nd + 1
            if(nd.gt.MAXDAT) then
                  write(*,*) ' ERROR: Exceeded memory for soft data'
                  stop
            end if
            x(nd) = xx
            y(nd) = yy
...
c
 ...
 202        format('Threshold: ',i2,' = ',f12.5)
      ...
      if(koption.eq.1) write(lout,203)
...
c                   **************************
c
c OPTIMIZED: OpenMP parallelization, multi-threshold LHS reuse,
...
c
      real, allocatable :: ccdfo_buf(:,:)
c
c Thread-private arrays (used in OpenMP parallel section):
c
      real      txa(MAXSAM),tya(MAXSAM),tza(MAXSAM),tvra(MAXSAM)
      real*8    ta(MAXEQ*(MAXEQ+1)/2),tr(MAXEQ),ts(MAXEQ)
      real      tccdf(MAXCUT),tccdfo(MAXCUT)
      real      tclose(MAXDAT)
      integer   tinfoct(8)
c
...
c
      ...
      write(*,*) 'Working on the kriging '
c
c Allocate output buffer for grid kriging:
c
      if(koption.eq.0) then
            allocate(ccdfo_buf(ncut,nxyz))
      end if
c
c MAIN LOOP OVER ALL THE BLOCKS IN THE GRID:
c OPTIMIZED: OpenMP parallelization with buffered output.
c For median IK, the LHS matrix is built only once per node
c (for the first cutoff) and reused for all thresholds via
c the kriging weights from the first solve.
c
      if(koption.eq.0) then
c
c --- OpenMP parallel grid IK ---
c   Use thread-local arrays since common block arrays can't be PRIVATE
c
!$OMP PARALLEL DO DEFAULT(SHARED) SCHEDULE(DYNAMIC,64)
!$OMP+PRIVATE(index,ix,iy,iz,xloc,yloc,zloc,nclose,nca,
!$OMP+  ic,krig,ia,j,ii,accept,neq,in,irot,i,k,is,ie,
!$OMP+  ising,cov,cmax,sumwts,
!$OMP+  txa,tya,tza,tvra,ta,tr,ts,tccdf,tccdfo,
!$OMP+  tclose,tinfoct)
!$OMP+REDUCTION(+:nk)
      do index=1,nxyz
c
...
c
                  ...
                  neq   = nca + ktype
c
c Build and solve the kriging system:
c
                  in   = 0
                  irot = 1 + (ic-1)*MAXNST
                  do j=1,nca
                        do i=1,j
                              in  = in + 1
                              call cova3(txa(i),tya(i),tza(i),txa(j),
     +                             tya(j),tza(j),ic,nst,MAXNST,c0,it,
     +                             cc,aa,irot,MAXROT,rotmat,cmax,cov)
                              ta(in) = dble(cov)
                        end do
                        call cova3(txa(j),tya(j),tza(j),xloc,yloc,
     +                             zloc,ic,nst,MAXNST,c0,it,cc,aa,
...
c Solve the system:
c
                  call ksol(1,neq,1,ta,tr,ts,ising)
c
...
c
                  ...
                  tccdf(ic) = 0.0
                  do i=1,nca
                        tccdf(ic) = tccdf(ic) + tvra(i)*real(ts(i))
                        sumwts    = sumwts    + real(ts(i))
                  end do
                  if(ktype.eq.0)
     +            tccdf(ic) = tccdf(ic) + (1.0-sumwts)*gcdf(ic)
c
...
c
            nk = nk + 1
            call ordrel(ivtype,ncut,tccdf,tccdfo,nviol,aviol,xviol)
c
c Store in buffer:
c
            do i=1,ncut
                  ccdfo_buf(i,index) = tccdfo(i)
            end do
            go to 16
c
c Unestimated node:
...
c
      do index=1,nxyz
            write(lout,'(30(f8.4))') (ccdfo_buf(i,index),i=1,ncut)
      end do
      deallocate(ccdfo_buf)
c
      else
c
c --- Sequential cross-validation path (koption > 0) ---
c
      do index=1,nloop
...
c
            do 12 ic=1,ncut
                  ...
                  do ia=1,nclose
                        ...
                        ii = int(actloc(j)+0.5)
                        accept = .true.
                        if((abs(x(j)-xloc)+abs(y(j)-yloc)+
     +                     abs(z(j)-zloc)).lt.EPSLON)
     +                  accept = .false.
                        if(vr(ii,ic).lt.tmin) accept = .false.
                        if(vr(ii,ic).gt.tmax) accept = .false.
                        if(accept) then
 ...
 13               continue
     ...
     +                             za(j),ic,nst,MAXNST,c0,it,cc,aa,
     +                             irot,MAXROT,rotmat,cmax,cov)
                              a(in) = dble(cov)
                        end do
