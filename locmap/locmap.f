...
c
c-----------------------------------------------------------------------
      parameter (MAXDAT= 15000, MAXH=500,
     +           BIGNUM=1.0e20,EPSLON=1.0e-20,VERSION=2.000)

      character  datafl*40,outfl*40,title*40,str*80,hex(16)*1,iline*2,
     +           jline(MAXH)*2,hexa*2,rline(MAXH)*2,gline(MAXH)*2,
     +           bline(MAXH)*2
      real       x(MAXDAT),y(MAXDAT),vr(MAXDAT),xx(5),yy(5),var(50)
      logical    testfl
c
...
c
      ...
      nd = 0
 2    read(lin,*,end=3) (var(j),j=1,nvari)
      vrt = var(ivrl)
      if(vrt.ge.tmin.and.vrt.lt.tmax) then
            nd     = nd + 1
            if(nd.gt.MAXDAT) then
                  write(*,*) 'ERROR: exceeded MAXDAT ',MAXDAT
                  stop
            end if
            x(nd)  = var(ixl)
            y(nd)  = var(iyl)
...
c
 ...
 4                continue
                  if(lost.gt.0) then
                        write(lpsout,115) ix,iy,str(1:lost)
 115                    format(i4,1x,i4,1x,' m (',a,') ctext')
                  end if
            end if
c
...
c
c
c Modified to draw a light grid of lines
c
c
c-----------------------------------------------------------------------
      parameter (EPS=0.001)
