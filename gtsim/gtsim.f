...
c Parameters:
c
      parameter(MAXCAT = 24, VERSION=2026.1)
c
c Variable declaration:
c
      real      cat(MAXCAT),pdf(MAXCAT),thres(MAXCAT),var(50)
      integer   ipcol(MAXCAT),nvarip(MAXCAT)
      character inpfl*40,outfl*40,propfl(MAXCAT)*40,str*40
      logical   testfl
c
...
c
      write(*,9999) VERSION
 9999 format(/' GTSIM - GSLIB2-OPT v',f7.1,' (64-bit, OpenMP)'/
...
c
      read(lin,'(a40)',err=98) inpfl
      call chknam(inpfl,40)
      write(*,*) ' data file = ',inpfl
      ...
      if(iprop.eq.1) then
            do i=1,ncat-1
                  read(lin,'(a40)',err=98) propfl(i)
                  call chknam(propfl(i),40)
                  write(*,*) ' file   for proportion = ',propfl(i)
...
c
      ...
      write(lout,100) str
 100  format('GTSIM output',a40,/,'1',/,'category')
c
c Global thresholds?
c
      cp = 0.0
...
c
      write(*,9998) VERSION
 9998 format(/' GTSIM - GSLIB2-OPT v',f7.1, ' Finished'/)
      stop
