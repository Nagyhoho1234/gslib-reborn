...
c
      write(*,9998) VERSION
 9998 format(/' GAM - GSLIB2-OPT v',f7.1, ' Finished'/)
      stop
      end
...
c
      write(*,9999) VERSION
 9999 format(/' GAM - GSLIB2-OPT v',f7.1,' (64-bit, OpenMP)'/
...
c$OMP+PRIVATE(ix,iy,iz,id,ixinc,iyinc,izinc,ix1,iy1,iz1,il,iv)
c$OMP+PRIVATE(it,i,index,vrt,vrh,vrhpr,vrtpr,tempvar)
c$OMP+SCHEDULE(DYNAMIC,16)
      do ix=1,nx
      do iy=1,ny
      do iz=1,nz
c
