...
c
      ...
      close(lout)
      write(*,9998) VERSION
 9998 format(/' KT3D - GSLIB2-OPT v',f7.1, ' Finished'/)
      stop
      end
...
c
      write(*,9999) VERSION
 9999 format(/' KT3D - GSLIB2-OPT v',f7.1,' (64-bit, OpenMP)'/
...
c
c OPTIMIZED: OpenMP parallelization, block covariance pre-computation,
...
c
      real, allocatable :: est_buf(:), estv_buf(:)
      real, allocatable :: cdf_buf(:,:)
      real, allocatable :: xv_buf(:),yv_buf(:),zv_buf(:)
      real, allocatable :: tv_buf(:)
      integer nxyz_buf
c
c Thread-private kriging arrays (used in OpenMP parallel section):
c
      real      txa(MAXSAM),tya(MAXSAM),tza(MAXSAM)
      real      tvra(MAXSAM),tvea(MAXSAM)
      real*8    ta(MAXEQ*MAXEQ),tr(MAXEQ),trr(MAXEQ),ts(MAXEQ)
      real      tcdf(MAXCUT)
      real      tclose(MAXDAT)
      integer   tna, tinfoct(8)
c
...
c
c Allocate output buffers for grid kriging (koption=0):
c
      nxyz_buf = nloop
      if(koption.eq.0) then
            allocate(est_buf(nxyz_buf))
            allocate(estv_buf(nxyz_buf))
            if(iktype.eq.1) allocate(cdf_buf(MAXCUT,nxyz_buf))
      end if
c
c MAIN LOOP OVER ALL THE BLOCKS IN THE GRID:
c For koption=0 (grid), we can run in parallel with OpenMP.
c For koption>0 (cross-validation), sequential due to file I/O.
c
      if(koption.eq.0) then
c
c --- OpenMP parallel grid kriging ---
c
      nk    = 0
      xk    = 0.0
      vk    = 0.0
...
!$OMP+PRIVATE(index,ix,iy,iz,xloc,yloc,zloc,est,estv,
!$OMP+  extest,resce,nclose,accept,ind,neq,im,
!$OMP+  ising,cb,cb1,cov,cmax,dx,dy,dz,first,kadim,ksdim,
!$OMP+  nrhs,nv,sumwt,wtmin,icut,i,j,k,
!$OMP+  txa,tya,tza,tvra,tvea,ta,tr,trr,ts,tcdf,tclose,
!$OMP+  tna,tinfoct)
!$OMP+REDUCTION(+:nk,xk,vk)
      do index=1,nloop
c
...
c (Note: external drift file must be pre-loaded for parallel; for now
c  external drift with koption=0 not parallelized -- fall through)
c
...
c
     ...
     +              nzsup,zmnsup,zsizsup,nclose,tclose,tinfoct)
c
c Load the nearest data in txa,tya,tza,tvra,tvea:
c
      tna = 0
      do i=1,nclose
            ind    = int(tclose(i)+0.5)
            accept = .true.
            if(accept) then
                  if(tna.lt.ndmax) then
                        tna = tna + 1
                        txa(tna)  = x(ind) - xloc + 0.5*xsiz
                        tya(tna)  = y(ind) - yloc + 0.5*ysiz
                        tza(tna)  = z(ind) - zloc + 0.5*zsiz
                        tvra(tna) = vr(ind)
                        tvea(tna) = ve(ind)
                  end if
            end if
...
c
      if(tna.lt.ndmin) then
            est_buf(index)  = UNEST
            estv_buf(index) = UNEST
            go to 11
      end if
c
c Test if there are enough samples to estimate all drift terms:
c
      if(tna.ge.1.and.tna.le.mdt) then
            est_buf(index)  = UNEST
            estv_buf(index) = UNEST
            go to 11
      end if
...
c
            call cova3(txa(1),tya(1),tza(1),txa(1),tya(1),tza(1),1,
     +                 nst,MAXNST,c0,it,cc,aa,1,MAXROT,rotmat,cmax,
     +                 cb1)
c
c Establish Right Hand Side Covariance:
c
            if(ndb.le.1) then
                  call cova3(txa(1),tya(1),tza(1),xdb(1),ydb(1),
     +                 zdb(1),1,nst,MAXNST,c0,it,cc,aa,1,MAXROT,
     +                 rotmat,cmax,cb)
            else
                  cb  = 0.0
                  do i=1,ndb
                        call cova3(txa(1),tya(1),tza(1),xdb(i),ydb(i),
...
c
      do i=1,tna
      do j=i,tna
            call cova3(txa(i),tya(i),tza(i),txa(j),tya(j),tza(j),1,
     +                 nst,MAXNST,c0,it,cc,aa,1,MAXROT,rotmat,cmax,
     +                 cov)
            ta(neq*(i-1)+j) = dble(cov)
            ta(neq*(j-1)+i) = dble(cov)
      end do
      end do
c
...
c
      if(neq.gt.tna) then
            do i=1,tna
                  ta(neq*(i-1)+tna+1) = dble(unbias)
                  ta(neq*tna+i)       = dble(unbias)
            end do
      endif
c
...
c
      do i=1,tna
            if(ndb.le.1) then
                  call cova3(txa(i),tya(i),tza(i),xdb(1),ydb(1),
     +                 zdb(1),1,nst,MAXNST,c0,it,cc,aa,1,MAXROT,
     +                 rotmat,cmax,cb)
            else
                  cb  = 0.0
                  do j=1,ndb
                        call cova3(txa(i),tya(i),tza(i),xdb(j),ydb(j),
     ...
     +                             MAXROT,rotmat,cmax,cov)
      ...
      if(neq.gt.tna) tr(tna+1) = dble(unbias)
c
...
c
      if(idrif(1).eq.1) then
            im=im+1
            do k=1,tna
                  ta(neq*(im-1)+k) = dble(txa(k)*resc)
                  ta(neq*(k-1)+im) = dble(txa(k)*resc)
            end do
            tr(im) = dble(bv(1))
      endif
c
...
c
      if(idrif(2).eq.1) then
            im=im+1
            do k=1,tna
                  ta(neq*(im-1)+k) = dble(tya(k)*resc)
                  ta(neq*(k-1)+im) = dble(tya(k)*resc)
            end do
            tr(im) = dble(bv(2))
      endif
c
...
c
      if(idrif(3).eq.1) then
            im=im+1
            do k=1,tna
                  ta(neq*(im-1)+k) = dble(tza(k)*resc)
                  ta(neq*(k-1)+im) = dble(tza(k)*resc)
            end do
            tr(im) = dble(bv(3))
      endif
c
...
c
      if(idrif(4).eq.1) then
            im=im+1
            do k=1,tna
                  ta(neq*(im-1)+k) = dble(txa(k)*txa(k)*resc)
                  ta(neq*(k-1)+im) = dble(txa(k)*txa(k)*resc)
            end do
            tr(im) = dble(bv(4))
      endif
c
...
c
      if(idrif(5).eq.1) then
            im=im+1
            do k=1,tna
                  ta(neq*(im-1)+k) = dble(tya(k)*tya(k)*resc)
                  ta(neq*(k-1)+im) = dble(tya(k)*tya(k)*resc)
            end do
            tr(im) = dble(bv(5))
      endif
c
...
c
      if(idrif(6).eq.1) then
            im=im+1
            do k=1,tna
                  ta(neq*(im-1)+k) = dble(tza(k)*tza(k)*resc)
                  ta(neq*(k-1)+im) = dble(tza(k)*tza(k)*resc)
            end do
            tr(im) = dble(bv(6))
      endif
c
...
c
      if(idrif(7).eq.1) then
            im=im+1
            do k=1,tna
                  ta(neq*(im-1)+k) = dble(txa(k)*tya(k)*resc)
                  ta(neq*(k-1)+im) = dble(txa(k)*tya(k)*resc)
            end do
            tr(im) = dble(bv(7))
      endif
c
...
c
      if(idrif(8).eq.1) then
            im=im+1
            do k=1,tna
                  ta(neq*(im-1)+k) = dble(txa(k)*tza(k)*resc)
                  ta(neq*(k-1)+im) = dble(txa(k)*tza(k)*resc)
            end do
            tr(im) = dble(bv(8))
      endif
c
...
c
      if(idrif(9).eq.1) then
            im=im+1
            do k=1,tna
                  ta(neq*(im-1)+k) = dble(tya(k)*tza(k)*resc)
                  ta(neq*(k-1)+im) = dble(tya(k)*tza(k)*resc)
            end do
            tr(im) = dble(bv(9))
      endif
c
...
c
      call ktsol(neq,nrhs,nv,ta,tr,ts,ising,maxeq)
c
...
c
      if(ising.ne.0) then
            ...
            estv = real(cbb)
            do j=1,neq
                  estv = estv - real(ts(j))*trr(j)
                  if(j.le.tna) then
                        if(ktype.eq.0) then
                              est = est + real(ts(j))*(tvra(j)-skmean)
                        else
                              est = est + real(ts(j))*tvra(j)
                        endif
                  endif
            end do
...
c
c Store results in buffer:
c
      est_buf(index)  = est
      estv_buf(index) = estv
c
c Handle IK-type distribution if needed:
c
      if(iktype.eq.1) then
            do icut=1,ncut
                  tcdf(icut) = -1.0
            end do
            if(est.ne.UNEST.and.tna.gt.1) then
                  wtmin = 1.0
                  do i=1,tna
                        if(ts(i).lt.wtmin) wtmin = ts(i)
                  end do
                  sumwt = 0.0
                  do i=1,tna
                        ts(i)  = ts(i) - wtmin
                  ...
                  do i=1,tna
                        ts(i) = ts(i) / max(0.00001,sumwt)
                  end do
                  if(sumwt.gt.0.00001) then
                        do icut=1,ncut
                              tcdf(icut) = 0.0
                              do i=1,tna
                                    if(tvra(i).le.cut(icut))
     +                              tcdf(icut)=tcdf(icut)+ts(i)
                              end do
                        end do
                  end if
            end if
            do icut=1,ncut
                  cdf_buf(icut,index) = tcdf(icut)
            end do
      end if
c
 11         continue
...
c
      do index=1,nloop
            if(iktype.eq.0) then
                  write(lout,'(f9.3,1x,f9.3)') est_buf(index),
     +                                          estv_buf(index)
            else
                  write(lout,'(30(f8.4))') (cdf_buf(i,index),i=1,ncut)
            end if
      end do
c
...
c
      deallocate(est_buf)
      deallocate(estv_buf)
      if(iktype.eq.1) deallocate(cdf_buf)
c
      else
c
c --- Sequential cross-validation / jackknife path (koption > 0) ---
c
      nk    = 0
      xk    = 0.0
      vk    = 0.0
...
c
      ...
      do i=1,nclose
            ind    = int(close(i)+0.5)
            accept = .true.
            if((abs(x(ind)-xloc)+abs(y(ind)-yloc)+abs(z(ind)-zloc))
...
c
     ...
     +                 MAXNST,c0,it,cc,aa,1,MAXROT,rotmat,cmax,cb1)
            if(ndb.le.1) then
...
c
c Set up and solve the kriging system (same as original):
c
      neq = mdt+na
      first = .false.
      do i=1,neq*neq
     ...
     +                 MAXNST,c0,it,cc,aa,1,MAXROT,rotmat,cmax,cov)
...
c
c Write output for cross-validation:
c
 1          continue
            err = UNEST
