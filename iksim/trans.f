...
c
c-----------------------------------------------------------------------
c Slightly modified by Zhanjun Ying to fit in program IKSIM.

      parameter(MAXREF=100000, MAXDAT=500000, MAXCAT=24, MV=20, 
     +          EPSLON=1.0e-12, VERSION=2.000)

      character distin*40,datafl*40,outfl*40,localfl*40,str*40
      real      rcdf(MAXREF),rvr(MAXREF),dcdf(MAXDAT),dvr(MAXDAT),
     +          indx(MAXDAT),fuzzcat(MAXDAT),catcdf(MAXCAT),var(MV),
     +          ltpar,utpar
      integer   ltail,utail,nx,ny,nz,wx,wy,wz,category(MAXCAT)
      logical   testfl
c
...
c
      write(*,*) ' Working on Transform ... '
      write(*,9999) VERSION
 9999 format(/' TRANS Version: ',f5.3/)

c
c The name of the parameter file is '__trans.par'
c

      open(lin,file='__trans.par',status='OLD')
c
...
c     delete tempory file __harddata.dat
      inquire(file='__harddata.dat', exist=testfl)
      if(testfl) then
         open(lin, FILE = '__harddata.dat', STATUS = 'OLD')
         close(lin, STATUS = 'DELETE')
      end if

c     delete tempory file __variance.out
      inquire(file='__variance.out', exist=testfl)
      if(testfl) then
         open(lin, FILE = '__variance.out', STATUS = 'OLD')
         close(lin, STATUS = 'DELETE')
      end if
