...
c                     ****************************
c
c ORIGINAL FORMAT VERSION:
c   - Outputs original input line + back-transformed value (nvari+1 columns)
c   - Matches gslib_ori output format
c   - Uses efficient single-pass I/O (read as string, parse from string)
...
c
      write(*,9999) VERSION
 9999 format(/' BACKTR - GSLIB2-OPT v',f7.1,' (64-bit, OpenMP)'/
...
c
c Read, transform and write in single pass (original format: line + bac)
c
      nrec = 0
...
c
      close(lout)
      write(*,9998) VERSION
 9998 format(/' BACKTR - GSLIB2-OPT v',f7.1, ' Finished'/)
      stop
