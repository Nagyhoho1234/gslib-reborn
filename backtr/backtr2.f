...
c
c OPTIMIZED VERSION:
c   - Eliminated backspace/re-read I/O pattern (3 ops -> 1 op per record)
c   - All data read into memory in single pass, processed in memory
...
c
c-----------------------------------------------------------------------
      parameter(MAXDAT=5000000,MAXLEN=512,EPSLON=0.00001,VERSION=2026.1)

      real      vr(MAXDAT),vrg(MAXDAT),var(50),ltpar,utpar,bac
      real*8    p
      character datafl*512,outfl*512,transfl*512,str*512
      integer   ltail,utail,nrec
...
c
      write(*,9999) VERSION
 9999 format(/' BACKTR2 - GSLIB2-OPT v',f7.1,' (64-bit, OpenMP)'/
...
c
c Now read the data file: first pass reads everything into memory
c
...
c
c OPTIMIZED: Read, transform and write in single pass
c
      nrec = 0
 7    read(lin,'(a)',end=8,err=99) str
      nrec = nrec + 1
      read(str,*,err=99) (var(i),i=1,nvari)
      if(getrank) then
            p = dble(var(ivr))
            call gauinv(p,var(ivr),ierr)
      end if
      bac = backtr(var(ivr),nt,vr,vrg,zmin,zmax,ltail,ltpar,
     +             utail,utpar)
      if(bac.lt.zmin) bac = zmin
      if(bac.gt.zmax) bac = zmax
      write(lout,'(f12.5)') bac
      go to 7
 8    continue
      close(lin)
      write(*,*) ' Processed ',nrec,' records'
c
c Finished:
c
      close(lout)
      write(*,9998) VERSION
 9998 format(/' BACKTR2 - GSLIB2-OPT v',f7.1, ' Finished'/)
      stop
