...
c
      write(*,9998) VERSION
 9998 format(/' HISTSMTH - GSLIB2-OPT v',f7.1, ' Finished'/)
      stop
      end
...
c-----------------------------------------------------------------------
      ...
      character datafl*40,outfl*40,psfl*40,title*40,str*512,xlab*12
      logical   testfl
c
...
c
      ...
      lps  = 4
      write(*,9999) VERSION
 9999 format(/' HISTSMTH - GSLIB2-OPT v',f7.1,' (64-bit, OpenMP)'/
...
c
      ...
      write(*,*) ' reference data file = ',datafl
      ...
      read(lin,'(a40)',err=97) psfl 
      call chknam(psfl,40)
      write(*,*) ' output PostScript file = ',psfl
      ...
      if(ilog.eq.1) then
            zmin = alog10(zmin+EPSLON)
            zmax = alog10(zmax+EPSLON)
      endif
