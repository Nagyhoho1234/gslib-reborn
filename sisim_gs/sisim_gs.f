...
c
      ...
      close(ldbg)
      write(*,9998) VERSION
 9998 format(/' SISIM_GS - GSLIB2-OPT v',f7.1, ' Finished'/)
      stop
      end
...
c-----------------------------------------------------------------------
      ...
      real      var(MV)
      real*8    p,acorni
      character datafl*40,tabfl*40,outfl*40,dbgfl*40,
     +          str*40,title*80,calfl*40,softfl*40
      logical   testfl
c
...
c
      write(*,9999) VERSION
 9999 format(/' SISIM_GS - GSLIB2-OPT v',f7.1,' (64-bit, OpenMP)'/
...
c
      ...
      read (*,'(a40)') str
      if(str(1:1).eq.' ')str='sisim_gs.par                           '
...
c
      ...
      if(ncutz.gt.MAXZCUT) stop 'ncutz is too big - modify .inc file'
      ...
      write(*,*) ' Calibration table file = ',calfl
...
c
      if(nxyz.gt.MXYZ) then
            ...
            write(*,*) '  Increase MXYZ in sisim_gs.inc'
            stop
      endif
c
...
c
      ...
      if(ncutv.gt.MAXVCUT) then
           write(*,*) 'ncutv is too big - modify .inc file'
           stop
      end if
      do i=1,ncutv
...
c-----------------------------------------------------------------------
      ...
      integer   nviolr(MAXZCUT,nsim)
      real      aviolr(MAXZCUT,nsim),xviolr(MAXZCUT,nsim)
      lin       =  1
c
...
c data number in the "tmp" array. All other nodes are informed by a
c soft datum:
c
            do i=1,nxyz
                  id = int(sim(i)+0.5)
...
c
                        call ordrel(ivtype,ncutz,ccdf,ccdfo,nviol,
     +                              aviol,xviol)
c
...
c
                  if(results(ind,isim).gt.UNEST) then
                        ...
                        do ic=1,ncutz
                              if(ivtype.eq.0) then
                                    if(results(ind,isim).eq.zcut(ic))
     +                                ccdf(ic)=ccdf(ic)+1.
                              else
                                    if(results(ind,isim).le.zcut(ic))
     +                                ccdf(ic)=ccdf(ic)+1.
                              end if
                        end do
                  endif
...
c
 ...
 302           format('     threshold',i2,' number = ',i6,
     +                ' average = ',f8.4,' maximum = ',f8.4)
            ...
            btot =(ntot / real(ncutz*max(nxysim,1))) * 100.0
...
c
c OUTPUT: ccdf(1..ncutz) filled with kriged indicator estimates
c
c-----------------------------------------------------------------------
      include  'sisim_gs.inc'
...
c
c Apply soft data corrections if needed:
c
            if(somesoft) then
                  in = 0
                  do j=1,na
...
c
     ...
     +                                    vraic = 0.0
                                    ...
                                    if(secval.le.vcut(1)) then
                                       ...
                                       do iv=2,ncutv
                                          if(secval.gt.vcut(iv-1).and.
     +                                       secval.le.vcut(iv))
     +                                       icutv = iv
                                       end do
                                    endif
                                    vraic = calib(icutv,ic)
                              end if
                        endif
...
c-----------------------------------------------------------------------
 ...
 57   format('1    0.15                      ',
     +       '-Five  nst, nugget effect')
      write(lun,58)
 58   format('1    0.85 0.0   0.0   0.0      ',
     +       '-      it,cc,ang1,ang2,ang3')
      write(lun,59)
 59   format('         10.0  10.0  10.0      ',
     +       '-      a_hmax, a_hmin, a_vert')
