...
c
      write(*,9998) VERSION
 9998 format(/' SGSIM - GSLIB2-OPT v',f7.1, ' Finished'/)
      stop
      end
...
c
      write(*,9999) VERSION
 9999 format(/' SGSIM - GSLIB2-OPT v',f7.1,' (64-bit, OpenMP)'/
...
c
c OMP: outbuf is in THREADPRIVATE /iobuf/ common block (see .inc)
c
c
...
c      Each realization needs a unique, reproducible ACORN seed state.
c      We advance the RNG for each realization and save the starting state.
c
      do isim=1,nsim
            do i=1,MAXOP1
...
c OMP: Parallel realization loop. Each thread gets its own copies of
c      THREADPRIVATE common blocks (iaco, simwrk, search, clooku,
c      krigev) via COPYIN. The sim array and RNG state are per-thread.
c
!$OMP PARALLEL DO
!$OMP+  PRIVATE(isim,ind,ix,iy,iz,in,index,id,id2,xx,yy,zz,
!$OMP+          test,test2,TINY,irepo,gmean,cmean,cstdev,lktype,
!$OMP+          p,xp,ierr,simval,ne,av,ss,imult,nnx,nny,nnz,
!$OMP+          jx,jy,jz,testind,infoct,var,cp,oldcp,w)
!$OMP+  COPYIN(/iaco/,/simwrk/,/search/,/clooku/,/krigev/,
!$OMP+         /iobuf/)
!$OMP+  SCHEDULE(DYNAMIC,1) ORDERED
      do isim=1,nsim
c
...
c (Note: secondary data re-reading is serial/skipped in parallel mode
c  because file I/O is not thread-safe. For ktype=4 with multiple
c  secondary models, run with OMP_NUM_THREADS=1)
c
...
c
c OMP: Buffered write - write entire realization at once (ordered)
c
!$OMP ORDERED
            do ind=1,nxyz
                  write(lout,'(f12.4)') outbuf(ind)
            end do
!$OMP END ORDERED
...
c
c OMP: Covariance table construction - the nested loops over
c      i,j,k can be partially vectorized. The cova3 calls are
c      independent for each (i,j,k) combination.
c
      nlooku = 0
      do i=-nctx,nctx
      xx = i * xsiz
...
c
      if(ising.ne.0) then
            if(idbg.ge.1) then
c                 write(ldbg,*) 'WARNING SGSIM: singular matrix'
c                 write(ldbg,*) '               for node',ix,iy,iz
            endif
            cmean  = gmean
            cstdev = 1.0
            return
...
c           write(ldbg,*) 'ERROR: Negative Variance: ',cstdev
            cstdev = 0.0
      endif
