...
c
c OPTIMIZED VERSION:
c   - Pre-compute trig products used multiple times
c   - Minimize redundant floating-point multiplications
c   - Thread-safe: no saved state, all locals automatic
c   - Called once per structure, so optimizations are minor
c
...
c-----------------------------------------------------------------------
     ...
     +          cosa,cosb,cost
c
c Local variables for pre-computed trig products:
c
      real*8    sinbcosa,sinbsina,sintsinb,costsinb
c
...
c
      ...
      cost  = dble(cos(theta))
c
c Pre-compute products used multiple times:
c
      sinbcosa = sinb*cosa
      sinbsina = sinb*sina
c
...
c
      afac1 = 1.0d0 / dble(max(anis1,EPSLON))
      afac2 = 1.0d0 / dble(max(anis2,EPSLON))
      rotmat(ind,1,1) =       (cosb * cosa)
      rotmat(ind,1,2) =       (cosb * sina)
      rotmat(ind,1,3) =       (-sinb)
      rotmat(ind,2,1) = afac1*(-cost*sina + sint*sinbcosa)
      rotmat(ind,2,2) = afac1*(cost*cosa + sint*sinbsina)
      rotmat(ind,2,3) = afac1*( sint * cosb)
      rotmat(ind,3,1) = afac2*(sint*sina + cost*sinbcosa)
      rotmat(ind,3,2) = afac2*(-sint*cosa + cost*sinbsina)
