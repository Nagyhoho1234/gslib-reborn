...
c
c OPTIMIZED VERSION:
c   - Pre-compute reciprocals to replace repeated divisions
c   - Cache diagonal element to avoid redundant array lookups
c   - Minimize index recomputation in inner loops
c   - Thread-safe: no saved state, all variables local/automatic
c   - Better memory access patterns in back-substitution
c
c
...
c
                  akdiag = a(kdiag)
                  if(abs(akdiag).lt.tol) then
                        ktilt=k
                        return
                  endif
c
c Compute multipliers — pre-compute reciprocal of pivot:
c
                  pivrecip = 1.0d0 / akdiag
                  i1 = kdiag
                  do i=kp1,n
                        i1    = i1+1
                        a(i1) = -a(i1)*pivrecip
...
c Cache a(j1) after interchange to avoid re-reading:
c
                  j1 = kdiag
                  j2 = npiv
                  do j=kp1,n
...
c
                        ...
                        kb    = nvb2-(il-1)*n-k+1
c
c Pre-compute reciprocal and negated solution value:
c
                        bkb   = b(kb)/a(kdiag)
                        b(kb) = bkb
                        t     = -bkb
