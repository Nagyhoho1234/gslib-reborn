...
c
c OPTIMIZED VERSION:
c   - Manhattan distance pre-filter before expensive sqdist call
c   - Pre-compute nxsup*nysup product to avoid repeated multiplication
c   - Cache super block grid bounds for fast rejection
c   - Thread-safe: no saved state, all variables local/automatic
c
c
...
c-----------------------------------------------------------------------
      ...
      logical inflag
c
c Local variables for Manhattan distance pre-filter:
c
      real    rad,ddx,ddy,ddz
c
c Pre-compute search radius for Manhattan distance filter.
c Manhattan distance >= Euclidean distance, but we use a generous
c factor since the rotation/anisotropy can scale differently per axis.
c Use sqrt(radsqd) as the isotropic cutoff — this is a cheap pre-filter
c that avoids the full rotation-matrix distance for obviously far points.
c
      rad = sqrt(radsqd)
c
c Pre-compute nxsup*nysup to avoid repeated multiplication:
c
      nxysp = nxsup*nysup
c
...
c
            ii = ixsup + (iysup-1)*nxsup + (izsup-1)*nxysp
            if(ii.eq.1) then
                  nums = nisb(ii)
                  i    = 0
            else
...
c
            do 2 ii=1,nums
                  i = i + 1
c
c Manhattan distance pre-filter: cheap rejection of obviously
c distant points before calling the expensive sqdist function.
c If any single coordinate difference exceeds the search radius,
c the anisotropic distance will also exceed it (conservative test).
c
                  ddx = abs(xloc - x(i))
                  ddy = abs(yloc - y(i))
                  ddz = abs(zloc - z(i))
                  if(ddx.gt.rad .or. ddy.gt.rad .or.
     +               ddz.gt.rad) go to 2
c
c Check squared distance (full anisotropic calculation):
c
