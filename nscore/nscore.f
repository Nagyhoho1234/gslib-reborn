...
c   - All data lines read into memory, processed in memory
c   - Output written in single buffered pass
c   - MAXDAT increased from 500000 to 5000000
c   - MAXREC added for in-memory line storage
c
c PROGRAM NOTES:
c
...
c
c-----------------------------------------------------------------------
      parameter(MAXDAT=5000000,MAXLEN=512,EPSLON=1.0e-6,VERSION=2026.1)
      parameter(MAXREC = 5000000)
c
c ACORN parameters:
c
...
c
     ...
     +          dpowint,doubone,acorni
      real      var(50)
      character datafl*512,outfl*512,smthfl*512,transfl*512,
     +          tmpfl*512,str*512
      character, allocatable :: lines(:)*512
      integer,   allocatable :: lostr(:)
...
c
      write(*,9999) VERSION
 9999 format(/' NSCORE - GSLIB2-OPT v',f7.1,' (64-bit, OpenMP)'/
...
c OPTIMIZED: Read all lines into memory first, then process
c
...
c
      allocate(lines(MAXREC))
      allocate(lostr(MAXREC))
      allocate(outval(MAXREC))
      nrec = 0
 70   read(lin,'(a)',end=71) str
      nrec = nrec + 1
...
c
      do irec=1,nrec
            read(lines(irec),*,err=99) (var(i),i=1,nvari)
c
...
c
                  ...
                  j   = min(max(1,j),(nd-1))
                  vrg = dpowint(vr(j),vr(j+1),wt_ns(j),wt_ns(j+1),
     +                          vrr,doubone)
            endif
c
...
c
            ...
            outval(irec) = real(vrg)
      end do
c
c OPTIMIZED: Write all output in single buffered pass
c
      do irec=1,nrec
            write(lout,'(a,1x,f12.5)') lines(irec)(1:lostr(irec)),
     +                                  outval(irec)
      end do
c
c Finished:
c
      close(lout)
      write(*,9998) VERSION
 9998 format(/' NSCORE - GSLIB2-OPT v',f7.1, ' Finished'/)
      stop
