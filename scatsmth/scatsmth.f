...
c-----------------------------------------------------------------------
     ...
     +       swath(MAXLIM,50),rslope(MAXLIM)
      ...
      character xsmthfl*80,ysmthfl*80,datafl*80,outfl*80,dbgfl*80,str*80
      real      tmpval(50)
      logical   testfl
c
...
c
      ...
      lps  = 4
      write(*,9999) VERSION
 9999 format(/' SCATSMTH - GSLIB2-OPT v',f7.1,' (64-bit, OpenMP)'/
...
c
      ...
      read (*,'(a40)') str
      if(str(1:1).eq.' ')str='scatsmth.par                            '
...
c
      ...
      read(lin,'(a80)',err=97) dbgfl 
      call chknam(dbgfl,80)
      write(*,*) ' debugging file = ',dbgfl(1:40)
      open(ldbg,file=dbgfl,status='UNKNOWN')

      read(lin,'(a80)',err=97) xoutfl 
      call chknam(xoutfl,80)
      write(*,*) ' X output file = ',xoutfl(1:40)

      read(lin,'(a80)',err=97) youtfl 
      call chknam(youtfl,80)
      write(*,*) ' Y output file = ',youtfl(1:40)

      read(lin,'(a80)',err=97) outfl 
      call chknam(outfl,80)
...
c
      ...
      write(lout,110) ncols
 110  format('Smooth Results',/,i1,/,'X-value',/,'Y-value')
